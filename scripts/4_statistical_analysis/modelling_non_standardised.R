...
# Putatively deleterious variants filtered with high impact SnpEff annotations
# Import the dataset:
...
snpeff_cleaned <-
  snpeff_dataset %>% 
  select(line, Trait, trait_value, Sex, a_total_load, x_total_load) %>% # keep these specified cols 
  rename(autosomes = a_total_load, # rename before pivoting
         x = x_total_load) %>% 
...
complete_model_young_snpeff <- brm(trait_value ~ 0 + genetic_load * Sex * chromosome + (1|line),
                                   ...
                                   seed = 1,
                                   file = "C:/path/coding_R/models/complete_model_young_snpeff_non_stand")
complete_model_young_snpeff <- readRDS("C:/path/coding_R/models/complete_model_young_snpeff_non_stand.rds")
...
complete_model_old_snpeff <- brm(trait_value ~ 0 + genetic_load * Sex * chromosome + (1|line),
                                 ...
                                 iter = 8000, warmup = 4000, chains = 4, cores = 4, # run specifics
                                 seed = 1,
                                 file = "C:/path/coding_R/models/complete_model_old_snpeff_non_stand")
complete_model_old_snpeff <- readRDS("C:/path/coding_R/models/complete_model_old_snpeff_non_stand.rds")
...
# Stack the dfs:
comb_long_snpeff <- rbind(posterior_sample_long_young_snpeff, posterior_sample_long_old_snpeff)

# Calculate the difference between posteriors of males/ females/ young/ old:
...
ggsave(filename = "C:/path/coding_R/data/plots/supplementary/single_effect_size_snpeff_non_stand.png",
       plot = posterior_dist_snpeff, width = 14, height = 8, units = "in", dpi = 300)
...
ggsave(filename = "C:/path/coding_R/data/plots/supplementary/single_diff_effect_size_snpeff_non_stand.png",
       plot = posterior_dist_diff_snpeff, width = 14, height = 8, units = "in", dpi = 300)

# After fitting the model, make predictions for combinations of explanatory variables
# Create new data set and populate it with genetic loadvalues of arbitrary values 
new_data <-
...
fitted_data_young <-
  fitted(complete_model_young_snpeff,
         newdata = new_data) %>%  # extracts values from the model and re-evaluates them on the new data set
  bind_cols(new_data)
...
young_labels_snpeff <- as_labeller(c("autosomes" = "A. Autosomes, early-life",
                                     'x' = "B. X chromosome, early-life"))
old_labels_snpeff <- as_labeller(c("autosomes" = "C. Autosomes, late-life",
...
min(snpeff_dataset_young$trait_value) # -4.3
max(snpeff_dataset_young$trait_value) # 2.6
min(snpeff_dataset_young$genetic_load) # 0.02
max(snpeff_dataset_young$genetic_load) # 0.09
...
min(snpeff_dataset_old$trait_value) # -3.2
max(snpeff_dataset_old$trait_value) # 2.2
min(snpeff_dataset_old$genetic_load) # 0.02
max(snpeff_dataset_old$genetic_load) # 0.09
...
ggsave(filename = "C:/path/coding_R/data/plots/supplementary/single_scatterplots_non_stand.png",
       plot = scatter_snpeff, width = 14, height = 8, units = "in", dpi = 300)

(all_snpeff <- (wrap_elements(full = scatter_snpeff) |
                  posterior_dist_snpeff |
                  posterior_dist_diff_snpeff) +
    plot_layout(widths = c(2, 1, 1)) +
    plot_annotation(title = "Impact of genetic load on fitness [SnpEff filter, non-standardised]",
                    theme = theme(plot.title = element_text(size = 20))))

ggsave(filename = "C:/path/coding_R/data/plots/comb_plots_snpeff_non_stand.png",
       plot = all_snpeff, width = 14, height = 8, units = "in", dpi = 300)

# single effect size plots
(single_effect_sizes_snpeff <- (posterior_dist_snpeff | posterior_dist_diff_snpeff))

ggsave(filename = "C:/path/coding_R/data/plots/supplementary/single_effect_sizes_snpeff_non_stand.png",
       plot = single_effect_sizes_snpeff, width = 14, height = 8, units = "in", dpi = 300)

## Modelling the 'Gerp data set', that is, ----
...
gerp_cleaned <- gerp_dataset %>% 
  select("line", "Trait", "trait_value", "Sex",  "a_total_load",
         "x_total_load") %>% 
  rename(autosomes = a_total_load,
         x = x_total_load) %>% 
...
complete_model_young_gerp <- brm(trait_value ~ 0 + genetic_load * Sex * chromosome + (1|line),
                                 ...
                                 iter = 8000, warmup = 4000, chains = 4, cores = 4, # run specifics
                                 seed = 1,
                                 file = "C:/path/coding_R/models/complete_model_young_gerp_non_stand")
complete_model_young_gerp <- readRDS("C:/path/coding_R/models/complete_model_young_gerp_non_stand.rds")
...
complete_model_old_gerp <- brm(trait_value ~ 0 + genetic_load * Sex * chromosome + (1|line),
                          ...
                          iter = 8000, warmup = 4000, chains = 4, cores = 4, # run specifics
                          seed = 1,
                          file = "C:/path/coding_R/models/complete_model_old_gerp_non_stand")
complete_model_old_gerp <- readRDS("C:/path/coding_R/models/complete_model_old_gerp_non_stand.rds")

# Posterior predictive checks => take the model output and run it 'ndraws'-times:
# The plots should look roughly the same
(posterior_predictive_check_young_gerp <-
    bayesplot::pp_check(complete_model_young_gerp,
                        type = "hist",
                        ndraws = 11,
                        binwidth = 0.1))

(posterior_predictive_check_old_gerp <-
...
(all_gerp <- (wrap_elements(full = scatter_gerp) |
                  ...
                  posterior_dist_diff_gerp) +
    plot_layout(widths = c(2, 1, 1)) +
    plot_annotation(title = "Impact of genetic load on fitness [GERP filter, non-standardised]",
                    theme = theme(plot.title = element_text(size = 20))))

ggsave(filename = "C:/path/coding_R/data/plots/comb_plots_gerp_non_stand.png",
