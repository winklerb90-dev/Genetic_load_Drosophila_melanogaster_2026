...
Afterwards the models can be validated using Leave-One-Out (loo) cross validation and compared by their ELPD (expected log-pointwise predictive density). 
For the datasets we used, the unstandardised values were estimated as better fitting compared to the z-scored values by the loo cross validation.
